...
There’s a lot of empty directories that you can delete if you are so
inclined, but I will leave them as they are. You will notice, aside from the
gitignore, favicon, and robots file, we are left with four files of interest:
package.json, gatsby-config.js, src/pages/index.js, and src/pages/404.js. The
package.json is pretty standard (and bare), so we’ll skip over that.
